...
####################################################################
# Cálcular ID estimada con cota superior de flujo.
# Calcular kout (grado de salida) y kin (grado de entrada)
kout <- aggregate(nod_ent ~ nod_sal, data = AD, length)
names(kout)[2] <- "kout"

kin <- aggregate(nod_sal ~ nod_ent, data = AD, length)
names(kin)[2] <- "kin"

# Combinar con la red original
I <- merge(AD_original, kout, by.x = "nod_sal", by.y = "nod_sal", all.x = TRUE)
I <- merge(AD_original, kin, by.x = "nod_ent", by.y = "nod_ent", all.x = TRUE)

# Calcular el mínimo entre kout y kin para cada arista
I$min_k <- pmin(I$kout, I$kin)

I <- I[, c("nod_sal", "nod_ent", "min_k")]

ID_est = matrix(0,n,n)

for (i in 1:N){
  ID_est[as.numeric( I[i,1]),as.numeric(I[i,2])] = as.numeric( I[i,3])
}


##################################################################
#                    PREPROCESAMIENTO FFEF                        #
################################################################## 
