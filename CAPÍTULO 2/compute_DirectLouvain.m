...
%
function [COMTY ending] = compute_DirectLouvain(M,s,self,debug,verbose)
...
%sum(M) suma los elementos de las columnas de M
%sum(sum(M) suma todos los elementos de M
...
%genero una lista de comunidades. Como inicialmente cada nodo esta en su
%propia comunidad, habra tantas comunidades como nodos
...
for k=1:N
 ...
 %(guarda las posicioes, en este caso, la columna (la fila es k))
 %nodos vecinos a k
  ...
  for i=1:N
    ...
    for j=1:length(NB) %recorre todos los nodos vecinos del nodo que se esta analizando, i
      ...
      if (G(Cj) == 0) %si la ganancia al unirse al cluster Cj es 0
        ...
        G(Cj) = (Ki_in/m - (Kin(i)*SumTotout(Cj)+Kout(i)*SumTotin(Cj))/(m*m)) 
        if (ddebug)
...
fprintf('ya se ha empezado 2 valor com');
 ...
 Mnew;
    ...
    [COMt e] = compute_DirectLouvain(Mnew,0,self,debug,verbose);
